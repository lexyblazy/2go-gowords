...
export default React.memo(function ActivityFeedItem({
  ...
  if (item.type === "otherPlayerSubmission") {
    return (
      <div className="flex items-baseline gap-2 text-sm animate-fade-up min-w-0 overflow-hidden">
        ...
        <span className="text-slate-700 dark:text-slate-300 font-medium min-w-0 truncate">
          {item.message}
        </span>
      </div>
    );
  ...
  if (item.type === "rules") {
    return (
      <div className="px-2 py-2 animate-fade-up min-w-0 overflow-hidden">
        <div
          className="
            w-full max-w-2xl min-w-0 mx-auto text-sm rounded-xl
